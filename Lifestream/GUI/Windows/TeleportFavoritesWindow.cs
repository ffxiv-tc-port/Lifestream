...
public class TeleportFavoritesWindow : Window
{
    ...
    /// 靜默少列幾筆會讓人以為我的最愛掉了。
    ///
    /// ⚠️ <see cref="Data.Config.Favorites"/> 是**三個互不相干的 id 空間**共用同一個 HashSet
    /// (這是既有的資料模型，不在這裡改)。台服 7.20 實測三者的範圍完全不重疊，所以可以靠 id
    /// 分辨是哪一種：
    ///   - <c>Aetheryte</c> 表：0..238 —— 傳送面板管的就是這一種。列不出來＝還沒共鳴。
    ///   - <c>HousingAethernet</c> 表：1966080..1966162 —— 住宅區乙太之光，由浮動視窗管。
    ///   - Lifestream 自訂(優雷卡/新月島)：69420100 起 —— 同樣由浮動視窗管。
    /// 後兩種**不是**「無法使用」，只是不屬於這個視窗，所以只報數量、不逐筆列，
